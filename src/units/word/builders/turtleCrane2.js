  ...
  return {
    draw(g, st, screen) {
      ...
      if (st.s1 > 0.99) {
        ...
        g.text(W / 2, -1.25, `${total}匹`, { size: 13, color: C2D.ink });
        g.text(W + 0.6, H / 2, `${legsBig}本`, { align: "left", size: 13, color: C2D.ink });
        if (st.s2 < 0.99) g.text(W / 2, H / 2, `${legsBig}×${total}`, { size: 15, color: C2D.ghost });
      }
      ...
      // --- 完成: 答え ---
      if (st.done > 0.5) {
        g.badge(nW / 2, H + 1.5, `つる ${ansTsuru}匹`, { bg: C2D.red });
        g.badge((nW + W) / 2, H + 1.5, `カメ ${ansKame}匹`, { bg: C2D.blue });
      }
    },
  };
}
